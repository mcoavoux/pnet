...
def print_data_distributions(dataset):
    ...
    print("Distribution, main labels: ", d_main / d_main.sum())
    for d in d_aux:
        print("Aux distribution : ", d / d.sum())
...
def train_one(trainer, bilstm, example, classifiers, aux=False):
    encoding, transducting = bilstm.build_representations(example.get_sentence(), training=True)
    if not aux:
...
def evaluate_one(bilstm, example, main_classifier, aux_classifiers, adversary=False):
    encoding, transducting = bilstm.build_representations(example.get_sentence(), training=False)
...
def evaluate(bilstm, dataset, main_classifier, aux_classifiers, adversary=False):
    loss = 0
    acc = 0
    if aux_classifiers is not None:
    ...
    for example in dataset:
        ...
        main, aux = evaluate_one(bilstm, example, main_classifier, aux_classifiers, adversary)
        if main != (None, None):
            loss += main[0]
            acc += main[1]
...
def train_model(trainer, bilstm, sentiment_classifier, epochs, train, dev, aux_classifiers=None, lr=0.001, dc=1e-6):
    ...
    for epoch in range(epochs):
        ...
        for i, example in enumerate(train):
            ...
            train_one(trainer, bilstm, example, sentiment_classifier, False)
        ...
        main_t, aux_t= evaluate(bilstm, sample_train, sentiment_classifier, aux_classifiers)
        main_d, aux_d = evaluate(bilstm, dev, sentiment_classifier, aux_classifiers)
...
def train_one_adversary(trainer, bilstm, example, sentiment_classifier, adversary_classifiers):
...
def train_adversary(trainer, bilstm, sentiment_classifier, adversary_classifiers, epochs, train, dev, lr=0.001, dc=1e-6):
    random.shuffle(train)
    sample_train = train[:len(dev)]
    trainer.learning_rate = lr
    n_updates = 0
    ...
    for epoch in range(epochs):
        ...
        for i, example in enumerate(train):
            ...
            train_one_adversary(trainer, bilstm, example, sentiment_classifier, adversary_classifiers)
            
            trainer.learning_rate = lr / (1 + n_updates * dc)
        
        sys.stderr.write("\r")
        
        main_t, aux_t= evaluate(bilstm, sample_train, sentiment_classifier, adversary_classifiers, adversary=True)
        main_d, aux_d = evaluate(bilstm, dev, sentiment_classifier, adversary_classifiers, adversary=True)
...
def main(args):
    ...
    train_model(trainer, bilstm, sentiment_classifier, args.iterations, train, dev, l, args.learning_rate, args.decay_constant)
    
    
    main_t, aux_t = evaluate(bilstm, test, sentiment_classifier, l, adversary=False)
    losst, acct = main_t
    ...
    train_adversary(trainer, bilstm, sentiment_classifier, adversary_classifiers, args.iterations_adversary, train, dev, args.learning_rate, args.decay_constant)

    main_t, aux_t = evaluate(bilstm, test, sentiment_classifier, adversary_classifiers, adversary=True)
    losst, acct = main_t
    losst, acct  = list(map(lambda x : round(x, 4), [losst, acct]))
    aux_train = " ".join(["l={} a={}".format(round(l, 4), round(a, 4)) for l, a in aux_t])
